...
import Button from "react-select2-native/lib/Button";
import TagItem from "react-select2-native/lib/TagItem";
import utilities from "react-select2-native/lib/utilities";
...
class Select2 extends Component {
  ...
  renderItem = ({ item, idx }) => {
    let { colorTheme, isSelectSingle, selectKet } = this.props;
    return (
      <TouchableOpacity
        key={idx}
      ...
      >
        <Text style={[styles.itemText, this.defaultFont]}>
          {selectKet !== undefined ? item.ket : item.name}
        </Text>
        {!isSelectSingle && (
          <Icon
  ...
  renderSinglePreselectedItem = () => {
    const { selectedTitleStyle, colorLabel } = this.props;
    ...
    return (
      <Text
        style={[
          ...
          selectedTitleStyle,
          {
            color: colorLabel != undefined ? colorLabel : '#333',
  ...
  renderSingleSelectedItem = () => {
    const { value, data, selectedTitleStyle, colorLabel } = this.props;
    ...
    return (
      <Text
        style={[
          ...
          selectedTitleStyle,
          {
            color: colorLabel != undefined ? colorLabel : 'black',
            fontFamily: fontsCustom.primary[400],
          },
        ]}
      >
  ...
  render() {
    ...
    let { show } = this.state;
    return (
      <TouchableOpacity onPress={this.showModal} activeOpacity={0.7} style={[styles.container]}>
        <Modal
